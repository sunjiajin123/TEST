...
class Mult(nn.Module):
    ...
    def forward(self,sequence_q,sequence_k,gate_q=None,gate_k=None):
        ...
        if gate_q==None:
            after_fuse,_ = self.transformerencoder(sequence_q, sequence_k, sequence_k)
        else:
            after_fuse,_ = self.transformerencoder(sequence_q, sequence_k, sequence_k, gate_q, gate_k)
        return after_fuse.permute(1,0,2),_


class RCM_5(nn.Module):

    def __init__(self, args):
        super(RCM_5, self).__init__()
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        fuse_1,_=self.fusenet1(text_h,audio_h,torch.squeeze(text_output_1),torch.squeeze(audio_output_1))
        ...
        fuse_2,_=self.fusenet2(fuse_1,vision_h,audio_fuse_output_1,vision_output_1)
