...
def run(args):
    ...
    args.model_save_path = os.path.join(args.model_save_dir,\
                                        f'{args.modelName}-{args.datasetName}-{args.train_mode}.pth')            #模型存放路径

    # indicate used gpu
    if len(args.gpu_ids) == 0 and torch.cuda.is_available():                                                     #设置gpu 不用看
        # load free-most gpu
        pynvml.nvmlInit()
    ...
    tmp_tensor = torch.zeros((100, 100)).to(args.device)                                                      #没用
    # load data and models
    dataloader = MMDataLoader(args)                                                                             #数据加载器
    model = AMIO(args).to(device)                                                                               #model
    del tmp_tensor
    def count_parameters(model):
        ...
        for p in model.parameters():
            if p.requires_grad:
                ...
                # print(p)
        return answer
    logger.info(f'The model has {count_parameters(model)} trainable parameters')                              #日志输出模型参数量
    # exit()
    # using multiple gpus
    ...
    atio = ATIO().getTrain(args)                                                                                        #创建模型训练对象
    # do train
    ...
    #     result=model(text, audio, vision)
    ...
    if args.is_tune:
        ...
        results = atio.do_test(model, dataloader['test'], mode="TEST")                                                  #模型+测试集     获得测试集结果
...
def run_normal(args):
    args.res_save_dir = os.path.join(args.res_save_dir, 'normals')                                         #将结果存放到results/20200506/normals文件夹
    init_args = args
    model_results = []
    seeds = args.seeds
    # run results
    for i, seed in enumerate(seeds):                                                                      #根据不同的随机种子重复5次
        args = init_args
        # load config
        if args.train_mode == "regression":                                                                 #根据回归还是分类，设置参数,ConfigRegression会返回我们所选模型的训练参数
            config = ConfigRegression(args)
        else:
            config = ConfigClassification(args)
        args = config.get_config()
        setup_seed(seed)                                                                                    #设置随机种子
        args.seed = seed
        logger.info('Start running %s...' %(args.modelName))                                                 #日志 开始
        logger.info(args)                                                                                    #日志 参数信息
        # runnning
        args.cur_time = i+1
        test_results = run(args)                                                                             #进入到run()函数，并返回结果
        # restore results
        model_results.append(test_results)                                                                   #保存结果
...
def parse_args():                                     #预参数
    ...
    parser.add_argument('--modelName', type=str, default='rcm_5',
