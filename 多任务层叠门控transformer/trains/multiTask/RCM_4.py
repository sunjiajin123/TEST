            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    for m in self.args.tasks:
                        ...
                        current_loss[m]=self.criterion(outputs[m], labels[m])
                        # if m=='T':
                        #     self.args[m]=torch.abs(current_loss[m].detach()-0.1)
                        # if m=='A':
                        #     self.args[m]=torch.abs(current_loss[m].detach()-0.3)
                        # if m=='V':
                        #     self.args[m] = torch.abs(current_loss[m].detach() - 0.5)
